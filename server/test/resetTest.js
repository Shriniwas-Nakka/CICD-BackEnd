...
describe("Test cases for reset password api status", function(){
    ...
    */
    it("Reset password set Status 200", function(){
        ...
        chai.request(server).post('/fundoo/resetpassword/:token').send(json.reset200).set('token',json.token).end((err, data) => {
    ...
    */
    it("Failed to reset password Status 400", function(){
        var json = readFile();
        chai.request(server).post('/fundoo/resetpassword/:token').send(json.reset400).set('token',json.token).end((err, data) => {
            expect(err).to.be.null;
    ...
    it("Reset Password Invalid Data Status 422", function(){
        var json = readFile();
        chai.request(server).post('/fundoo/resetpassword/:token').send(json.reset422).set('token',json.token).end((err, data) => {
            expect(err).to.be.null;
