...
describe("Test cases for email verification api status", function(){
    ...
    */
    it("Email Verification Successful Status 200", function(){
        ...
        chai.request(server).post('/fundoo/verify/:token').set('token',json.emailverify200.token).end((err, data) => {
    ...
    */
    it("Invalid Email Verification data Status 400", function(){
        var json = readFile();
        chai.request(server).post('/fundoo/verify/:token').set('token',json.emailverify400.token).end((err, data) => {
            expect(err).to.be.null;
